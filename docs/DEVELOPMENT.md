...
- **Dove sta l'APK**: `public/tvBoss.apk`, servito dal nostro dominio. Non su GitHub Releases, che sarebbe stato più pulito ma non funziona: `github.com` dichiara App Links per `com.github.android`, quindi su Android il link di download veniva dirottato all'app GitHub invece di scaricare il file (e un ripiego via `fetch` non è possibile: gli asset GitHub non mandano CORS). Il `.gitignore` ignora `*.apk` con l'eccezione `!public/*.apk`. Sostituendolo vanno aggiornati anche `version` e `sizeBytes` in `lib/apk.ts`
...
- [x] **APK scaricabile da Impostazioni** (Fase 4, piano completato): la card mostra `tvBoss.apk · v1.0.0.0 · 1,9 MB` e compare solo su Android, fuori da standalone/TWA, a chi non ha già l'app. L'APK è servito da `public/tvBoss.apk`, **non** da GitHub Releases: `github.com` dichiara App Links per l'app GitHub, che dirottava il download. La release `v1.0.0` resta come archivio e per l'AAB
